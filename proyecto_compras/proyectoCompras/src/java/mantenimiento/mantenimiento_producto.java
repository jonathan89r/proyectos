...
public class mantenimiento_producto {
    ...
    public List consultarCodigo(String codigo) {
        EntityManager em = jpautil.getEntityManagerFactory().createEntityManager();
        em.getTransaction().begin();
        List<TabProducto> listaProveedor = new ArrayList<>();
        try {
            Query q = em.createNamedQuery("TabProducto.findByCodigo").setParameter("codigo", codigo);
            listaProveedor = q.getResultList();
            em.getTransaction().commit();
        } catch (Exception e) {
            em.getTransaction().rollback();
    ...
    public TabProducto consultarProducto(String t) {
        EntityManager em = jpautil.getEntityManagerFactory().createEntityManager();
        em.getTransaction().begin();

        Query q = em.createQuery("SELECT t FROM TabProducto t WHERE t.nombre = :nombre");
        q.setParameter("nombre", t);

        TabProducto c = null;

        List<TabProducto> lista = q.getResultList();

        for (TabProducto tabProducto : lista) {
            c = tabProducto;
        }
