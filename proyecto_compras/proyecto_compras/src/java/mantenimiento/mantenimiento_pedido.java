...
public class mantenimiento_pedido {
    ...
    public List<TabPedido> mostrar() {
        em.getTransaction().begin();
        try {
            query = em.createNamedQuery("TabPedido.findAll");
    ...
    public List consultarDatosPedido() {
        EntityManager emn = jpautil.getEntityManagerFactory().createEntityManager();
        emn.getTransaction().begin();
        List<TabPedido> lista = new ArrayList<>();
        try {
            Query q = emn.createQuery("SELECT t FROM TabPedido t");
